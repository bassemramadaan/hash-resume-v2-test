...
import { usePdfExportProgressStore } from "../store/usePdfExportProgressStore";
...
export async function exportResumeToPdf(
  ...
  const { startExportProgress, setExportStep, completeExport, setExportError, closeExportProgress } =
    usePdfExportProgressStore.getState();
  ...
  try {
    ...
    // Step 1: Preparing Fonts & Assets
    setExportStep('preparing_fonts', 20);
    ...
    try {
      ...
      // Step 2: Rendering Canvas & ATS layout
      setExportStep('rendering_canvas', 50);
      ...
      // Step 3: Slicing Pages & Building PDF
      setExportStep('slicing_pages', 80);
      await new Promise((r) => setTimeout(r, 60));
    ...
    setExportError(err?.message || 'حدث خطأ أثناء تجهيز وتصدير السيرة الذاتية');
    throw err;
  }
}
